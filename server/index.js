...
// Transliteration endpoint — converts English name to Hindi Devanagari script
app.post('/api/transliterate', async (req, res) => {
  try {
    ...
    if (!GROQ_API_KEY) {
      return res.status(500).json({ hindiName: name });
    }
    ...
    const groqResponse = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a transliteration tool. When given an English name, you MUST respond with ONLY the Hindi Devanagari transliteration of that name — no explanation, no punctuation, no extra words. Just the name in Devanagari script.',
          },
          {
            role: 'user',
            content: `Transliterate this name to Hindi Devanagari script: ${name}`,
          },
        ],
        temperature: 0.1,
        max_tokens: 20,
    ...
    if (!groqResponse.ok) {
      return res.json({ hindiName: name });
    }

    const data = await groqResponse.json();
    const hindiName = data?.choices?.[0]?.message?.content?.trim() || name;
    console.log(`[Transliterate] "${name}" → "${hindiName}"`);
    return res.json({ hindiName });
  } catch (error) {
    console.error('[Transliterate] Error:', error);
    return res.json({ hindiName: name }); // fallback to original
  }
});
